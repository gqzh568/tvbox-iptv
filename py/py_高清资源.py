...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		xmlTxt=self.custom_webReadFile(urlStr='https://1080zyk15.com/inc/api.php?ac=list&h=24')
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		limit=20
		total=9999
		Url='https://1080zyk15.com/inc/api.php?ac=list&t={0}&pg={1}'.format(tid,pg)
	...
	def detailContent(self,array):
		...
		vod_play_url=[]
		try:
			# url='https://1080zyk15.com/inc/apijson.php?ac=detail&ids='+id#https://1080zyk.com/?m=vod-detail-id-64171.html
			# xmlTxt=self.custom_webReadFile(urlStr=url)
			...
			url='https://1080zyk.com/?m=vod-detail-id-{0}.html'.format(id)
			htmlTxt=self.custom_webReadFile(urlStr=url)
			root = self.html(htmlTxt)
			ul=[root.xpath('//div[@id="play_1"]/ul'),root.xpath('//div[@id="play_2"]/ul')]
			vod_year=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'上映：<span>(\w{4})</span>',Index=1)
			vod_actor=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'主演：<span>(.*?)</span>',Index=1)
			vod_director=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'导演：<span>(.*?)</span>',Index=1)
			vod_area=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'地区：<span>(.*?\s*)</span>',Index=1)
			vod_lang=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'语言：<span>(.*?\s*)</span>',Index=1)
			try:
				vod_content=root.xpath('//div[@class="vodplayinfo"]/text()')[0]
				type_name=root.xpath('//div[@class="vodinfobox"]/ul/li[4]/span/text()')[0]
				# vod_area=self.custom_RegexGetText(Text=htmlTxt,RegexText=r'地区：<span>(.*?\s*)</span>',Index=1)
			except:
				pass
			for v in ul:
				...
				for  vod in nodes:
					a=vod.xpath('./a')
					if len(a)>0 and len(a[0].xpath('./input/@value'))>0:
						value=a[0].xpath('./text()')[0]
						href=a[0].xpath('./input/@value')[0]
						vodItems.append(value)
				joinStr = "#".join(vodItems)
				vodItems=[]
				vod_play_url.append(joinStr)
	...
	def searchContent(self,key,quick,pg='1'):
		...
		if int(pg)>self.searchpage:
			return {'list':[]}
		Url='https://1080zyk15.com/inc/api.php?ac=list&wd={0}&pg={1}'.format(urllib.parse.quote(key),pg)
		xmlTxt=self.custom_webReadFile(urlStr=Url)
		tree = et(fromstring(xmlTxt))
		root = tree.getroot()
		listXml=root.iter('list')
	...
	def custom_list(self,html):
		...
		if len(temporary)>0:
			...
			if len(idTxt)>1:
				idTxt=idTxt[0:-1]
				url='https://1080zyk15.com/inc/apijson.php?ac=detail&ids='+idTxt
				xmlTxt=self.custom_webReadFile(urlStr=url)
				jRoot = json.loads(xmlTxt)
				if jRoot['code']!=1:
					return videos
	...
	def custom_classification(self):
		xmlTxt=self.custom_webReadFile(urlStr='https://1080zyk15.com/inc/api.php?ac=list')
		tree = et(fromstring(xmlTxt))
		root = tree.getroot()
		...
		for vod in classXml:
			for value in vod:
				if self.custom_RegexGetText(Text=value.text,RegexText=self.FilterWords,Index=1)!='':
					continue
